...
function deduplicateConcepts(concepts) {
  ...
  for (const concept of concepts) {
    ...
    const isDup = seen.some(s => {
      ...
      return (sn.includes(norm) || norm.includes(sn)) && Math.abs(norm.length - sn.length) < 10
    })
...
function buildSummaryPrompt(concepts, subject) {
  const list = concepts.map((c, i) => `${i + 1}. [${c.category}] ${c.title}`).join('\n')
  return `
...
export async function extractConcepts(rawText, subject) {
  ...
  console.log(`[extractConcepts] ${chunks.length} chunk(s) for subject: ${subject}`)
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      const parsed = await callGeminiJson(prompt, { temperature: 0.2, maxTokens: 4096 })
    ...
    // Respect free-tier RPM limit (15 req/min = one every 4s)
  ...
  if (allConcepts.length === 0) {
    throw new Error('No concepts could be extracted from any chunk. The PDF may be image-only or the content may be unrecognised.')
  }

  const deduplicated = deduplicateConcepts(allConcepts)
  deduplicated.sort((a, b) => (b.exam_probability ?? 3) - (a.exam_probability ?? 3))

  console.log(`[extractConcepts] ${allConcepts.length} raw → ${deduplicated.length} after dedup`)
  ...
  const avgComplexity = deduplicated.reduce((s, c) => s + (c.complexity || 3), 0) / deduplicated.length
  const reading_level = avgComplexity <= 2 ? 'beginner' : avgComplexity <= 3.5 ? 'intermediate' : 'advanced'
